...
const FeaturesGrid = () => {
  const toruKnotRef = useRef(null);
  const firstHemisphereRef = useRef(null);

  const { scrollYProgress: torusKnotScrollYPosition } = useScroll({
    target: toruKnotRef,
    offset: ["start end", "end start"],
  });

  const torusKnotTranslateY = useTransform(
    torusKnotScrollYPosition,
    [0, 1],
    [100, -100]
  );
  ...
  const { scrollYProgress: firstHemisphereScrollYPosition } = useScroll({
    target: firstHemisphereRef,
    offset: ["start end", "end start"],
  });

  const firstHemisphereTranslateY = useTransform(
    firstHemisphereScrollYPosition,
    [0, 1],
    [50, -50]
  );
  ...
  const { scrollYProgress: coneScrollYPosition } = useScroll({
    target: coneRef,
    offset: ["start end", "end start"],
  });

  const coneTranslateY = useTransform(coneScrollYPosition, [0, 1], [100, -100]);
  const coneRotate = useTransform(coneScrollYPosition, [0, 1], [12, 45]);

  const secondHemisphereRef = useRef(null);

  const { scrollYProgress: secondHemisphereScrollYPosition } = useScroll({
    target: secondHemisphereRef,
    offset: ["start end", "end start"],
  });

  const secondHemisphereTranslateY = useTransform(
    secondHemisphereScrollYPosition,
    [0, 1],
    [50, -50]
  );
